...
export const Services = () => {
  ...
  return <section id="servicii" className="py-20 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-slate-800 mb-6">
            Servicii Funerare Turda
          </h2>
          <p className="text-xl text-slate-600 max-w-3xl mx-auto">Oferim servicii funerare în Turda, Câmpia Turzii, Mihai Viteazu, Viișoara și Tureni</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => <Card key={index} className={`h-full transition-all hover:shadow-lg ${service.highlight ? 'ring-2 ring-slate-800' : ''}`}>
              <CardHeader className="text-center pb-4">
                ...
                <CardTitle className="text-xl font-bold text-slate-800">
                  {service.title}
                  {service.highlight && <span className="block text-sm text-slate-600 font-normal mt-1">⭐ Serviciu Principal</span>}
                </CardTitle>
              </CardHeader>
              <CardContent>
